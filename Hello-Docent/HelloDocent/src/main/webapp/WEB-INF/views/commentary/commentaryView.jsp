...
<script src=" https://code.jquery.com/jquery-3.3.1.min.js"></script>    
            ...
            <div class="col-md-10 ml-auto mr-auto">
              ...
              <c:forEach var="item" items='${tags}'>
                <span class="badge badge-warning main-tag" style="margin-bottom:20px">${item}</span>
                      ...
                      <h6 class="title-uppercase"><fmt:formatDate value="${comInfo.CMT_DATE}" type="both" pattern="yyyy년 MM월 dd일 HH:mm"/></h6></div>
            </div>
                  ...
                  <div class="media">
                    <a class="pull-left" href="#paper-kit">
                      <div class="avatar big-avatar">
                        <img id="writer_photo" class="media-object" alt="64x64" onError='this.onerror=null;this.src="${pageContext.request.contextPath}/resources/assets/img/defaults/noimage1.png"' src="${pageContext.request.contextPath}/resources/assets/img/faces/kaci-baum-2.jpg">
                      </div>
                    </a>
                    
                    
                    <div class="media-body">
                      <h4 class="media-heading" id="writer_nick"></h4>
                      <div class="pull-right">
                      </div>
                      <p id="writer_introduce"></p>
      		...
      		<!-------------------- 대댓글 Dummy ------------------->
                ...
                <div class="media" id="refReplyDummy" style="display:none;" >
  				 ...
  				 <div class="row">
                        ...
                        <div class="col-12" style="margin:5px">
                        <div id="refReplyContent"></div></div></div>  
          ...
          <div class="row">
            <div class="related-articles w-100">
              ...
              <div class="container w-100" >
                <div class="row w-100" id="relatedContainer">
                  <div id="relatedBody" class="col-md-4" style="display:none">
                 
                      <img id="relatedImg" src="" alt="..." onError='this.onerror=null;this.src="${pageContext.request.contextPath}/resources/assets/img/defaults/noimage1.png"' class="img-rounded img-responsive">
 ...
 var action = function(element, response){
  ...
  $(document).ready(function(){
	...
	function initControlPanel(){
		...
		if('${sessionScope.userVO.user_iuid}' === '${comInfo.CMT_WRITERUID}')
		{ $("#control-panel").attr("style","inline"); }
		
		$("#control-panel").find("#comModifyBtn").on('click', function(){
		
			location.replace("/commentary/commentaryModifyView?comId="+'${comInfo.CMT_ID}'+"&dispId="+'${comInfo.CMT_REF}');
		...
		$("#control-panel").find("#comRemoveBtn").on('click', function(){
			...
			var isFree = '${comInfo.CMT_ISFREE}';
			if(isFree==='cost'){
				
				alert("유료 컨텐츠는 삭제할 수 없습니다. 관리자에게 문의하십시오.");
				return;
			}
			
			var confirm = window.confirm("정말 삭제하시겠습니까? 관련된 모든 정보가 사라집니다.");
			if(confirm){
			location.replace("/commentary/commentaryDelete?id="+'${comInfo.CMT_ID}');}
			else{
  ...
  function postReply(repRef){
	  ...
	  var reply = {
				...
				CMTR_ID : "",
				CMTR_REF : '${comInfo.CMT_ID}',
				CMTR_REPREF : repRef,
				CMTR_WRITERUID : '${sessionScope.userVO.user_iuid}',
				CMTR_CONTENT : $("#postReplyContent").val(),
				CMTR_DATE : new Date()
			...
			$.ajax({
				...
				type:"POST",
				url : "/reply/insertCommentaryReply",
  ...
  function setWriterProfile(){
	  
	  
	 var $writerPhoto = $("#writer_photo");
	 var $writerNick = $("#writer_nick");
	 var $writerIntro = $("#writer_introduce");
	 

		var action = function(element, response){
			
			  element.find("#writer_photo").attr("src", response.USER_PROFILEPHOTO);
			  element.find("#writer_nick").html(response.USER_NICK);
				
		}
		
		console.log("WRITERUID : " + '${comInfo.CMT_WRITERUID}' );
		
		getMemberPhoto($(document), '${comInfo.CMT_WRITERUID}' ,action);
	  ...
	  if(!content.trim()){
		  ...
		  return;
			...
			$.ajax({
				...
				type:"POST",
				url : "/reply/updateCommentaryReply",
				data : JSON.stringify(reply),
				contentType:"application/json",
				success : function(response){
  ...
  function getReplys(){
	  ...
	  $.ajax({
		  ...
		  url : "/reply/selectCommentaryReply",
		  type : "GET",
		  data : {id : '${comInfo.CMT_ID}'},
		  success : function(response){
  ...
  function renderingHandler(container){
	  ...
	  for(var i=0; i<replyList.length; ++i){
		  ...
		  if(keySet.includes(replyList[i].cmtr_ID)){
			  
			  console.log("keySet에 들어있습니다.. ");
			  
			  renderingRefOriginReplys($refReplyOriginDummy, $field, $replyArea, replyList[i], calMyDoc(replyList[i].cmtr_WRITERUID));
			  
		  
		  }else{
			  
			  
			  renderingReplys($dummyReply, $field, $replyArea ,replyList[i], calMyDoc(replyList[i].cmtr_WRITERUID));
		...
		for(var i=0; i<keySet.length; ++i){
			...
			for(var j=0; j<refReps.length; ++j){
				...
				renderingRefReplys($refReplyDummy, $ref, $replyArea ,refReps[j], calMyDoc(refReps[j].cmtr_WRITERUID));
  ...
  function renderingReplys($component, $field, $postArea, reply, isMyDoc){
	  ...
	  $instance.attr("id", reply.cmtr_ID);
	  $instance.attr("style", "display:block");
	  $instance.find("#reply_date").html(getFormatDate(new Date(reply.cmtr_DATE)));
	  $instance.find("#reply_content").html(reply.cmtr_CONTENT);
	  getMemberPhoto($instance, reply.cmtr_WRITERUID, action);
  ...
  function renderingRefOriginReplys($component, $field, $postArea, reply, isMyDoc){
	  ...
	  $instance.attr("id", reply.cmtr_ID);
	  $instance.attr("style", "display:block");
	  $instance.find("#refReplyOriginDate").html(getFormatDate(new Date(reply.cmtr_DATE)));
	  $instance.find("#refReplyOriginContent").html(reply.cmtr_CONTENT);
	  getMemberPhoto($instance, reply.cmtr_WRITERUID, action);
  ...
  function renderingRefReplys($component, $ref, $postArea, reply, isMyDoc){
	  ...
	  $instance.attr("id", reply.cmtr_ID);
	  $instance.attr("style", "display:block");
	  $instance.find("#refReplyDate").html(getFormatDate(new Date(reply.cmtr_DATE)));
	  $instance.find("#refReplyContent").html(reply.cmtr_CONTENT);
	  getMemberPhoto($instance, reply.cmtr_WRITERUID, action);
	  ...
	  $myComponent.find("#reply_modifyBtn").on('click', function(){
		  ...
		  var $replyPostArea = $replyArea.clone();
			 ...
			 if($instance.find("#replyPost").length > 0){
				 ...
				 $replyPostArea.find("#postReplyContent").html(replyInfo.cmtr_CONTENT);
				 $instance.append($replyPostArea);
 	  ...
 	  $myComponent.find("#reply_removeBtn").on('click', function(){
 		  ...
 		  if(confirm){
 		 requestRemove(replyInfo.cmtr_ID);
 		  }else{
 			  return;
 		  }
	  });
  ...
  function requestRemove(id){
	  ...
	  $.ajax({
		  ...
		  url : "/reply/removeCommentaryReply",
		  type : "GET",
		  data : {id : id},
		  success : function(response){
		  ...
		  error : function(xhr, status, error){
			  ...
			  console.log("댓글 삭제 오류 : " + status);
			  alert("댓글을 삭제하던 도중에 오류가 발생 하였습니다. ");
		  }
  ...
  function setDocentInfo(){
	  
	  
	  $.ajax({
		  
		 url : "/docent/getDocentInfoData",
		 type : "GET",
		 data : {id : encodeURIComponent("${comInfo.CMT_WRITERUID}")},
		 success : function(response){
			 console.log("도슨트 정보");
			 console.log(response);
			 $("#writer_introduce").html(response.di_introduce);
		 ...
		 error : function(status){
			 ...
			 console.log("Docent Info reciving error.. status --> " + status);
  ...
  function getFormatDate(date){
	  ...
	  function getMemberPhoto(element, iuid, action){
		  ...
		  $.ajax({
			  ...
			  success : function(posting){
				  ...
				  var postObj = posting[i];
					   ...
					   if(posting.cmt_ID === '${comInfo.CMT_ID}')
						  continue;
				  ...
				  $postNode.find("#relatedTitle").html(postObj.cmt_TITLE);
				
				$postNode.on('click', function(){
					console.log("url id : " + postObj.cmt_ID);
						var url = "/commentary/commentaryView?id=" + postObj.cmt_ID;
						console.log("url " + url);
						location.href=url;
