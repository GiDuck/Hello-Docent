	...
	<div class="main">
		...
		<c:set var="image_container" value="null" />
			...
			<c:forEach var="event" items="${events}">
					...
					<div class="col-md-3 col-sm-6 ml-auto" id="event_element">
			...
			<!-- Progress bar (기본값 hidden) -->
				...
				<div class="row" style="visibility:hidden" id="interval_progressBar">
...
/* 	$.fn.datetimepicker.dates['kr'] = {
		...
		$("#event_search_btn").on('click', function(){
			...
			if(!selected_type){
				alert("전시 타입을 선택 해 주세요.");
				return;
			}else if(!selected_place || selected_place==="전시관"){
		...
		function refreshUI(result){
			...
			var list = result;
			$container.empty();
			$("#interval_progressBar").attr("style", "visibility:visible");
			...
			for(var i = 0; i<list.length; ++i){
				...
				$element.find("#event_url").attr("herf", list[i].de_url);
			...
			$("#interval_progressBar").attr("style", "visibility:hidden");
