	...
	<div class="section section-dark">
		<div class="container">
			<div class="row">
				...
				<div class="row w-100" id="display_field">
...
function attachBtn(){
	...
	function renderDisplays(data){
		...
		var card = '<div class="col-md-3 col-sm-3"><div class="card card-product card-plain "> '+
			'<div class="card-image"><img src="" onError = this.onerror=null;this.src="${pageContext.request.contextPath}/resources/assets/img/defaults/noimage1.png" alt="Rounded Image" class="img-rounded img-responsive w-100"><div class="card-body">'+
					'<div class="card-description"><h5 id="title" class="card-title"></h5></div></div></div></div></div>';
					
					
		var $card = $(card);
		var $root = $("#display_field");
		
		for(var i = 0; i< data.length; ++i){
			
			var index = data[i];
			
			var $instance = $card.clone();
			
			$instance.find("img").attr("src", index.DPL_IMAGEURL);
			$instance.find("#title").html(index.DP_NAME);
			attachMoveEvent($instance, index.DP_ID);
	...
	function attachMoveEvent($instance, id){
		
		$instance.on('click',function(){
			
			location.href = "/display/findDisplayDetail?id="+id;
	 ...
	 function getDisplayInfo(){
		 ...
		 $.ajax({
			...
			url : "/display/getDisplaySimpleInfo",
			type : "GET",
			data : {museumId : '${container.EP_ID}'},
